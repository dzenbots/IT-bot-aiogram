...
from keyboards.inline import get_groups_list_to_add_keyboard, get_main_inline_keyboard
...
async def choose_group_to_add_user_to(call: CallbackQuery, callback_data: dict):
    if await check_valid_tuser(message=call.message, group_name='Admins'):
        ...
        await call.message.edit_reply_markup(
            reply_markup=get_groups_list_to_add_keyboard(user_id_to_add=user_id_to_add))
...
async def add_tuser_to_group(call: CallbackQuery, callback_data: dict):
    if await check_valid_tuser(message=call.message, group_name='Admins'):
        ...
        await call.message.edit_text(
            text=f'Пользователь {user.telegram_id} авторизован и добавлен в группу {group.group_name}')
